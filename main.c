...
int main()
{
    ...
    if (file == NULL) //если открыть не получилось, то закрываемс€
    {
    ...
    while (C[i] != EOF) //пока не встретим конец файла, выполн€ем цикл
    {
        i++;
    ...
    while (C[i] != EOF) //вывод оставшегос€ массива (ничего не должно остатьс€)
    {
        printf("%c", C[i]);
        i++;
    }
